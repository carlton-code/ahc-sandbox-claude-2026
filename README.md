...
`Customer` (CRUD, name-search, order/customer summaries, rewards, and address endpoints),
`Product` (CRUD), and `Order` (read-only: list with an optional `?customerId=` filter, and by-id
with line items).
